...
games_scores_get_current (GamesScores * self)
{
  ...
  if (cat->backend == NULL) {
    cat->backend = _games_scores_backend_new (priv->style, priv->basename,
                                             cat->category.key);
  }
...
games_scores_add_score (GamesScores * self, GamesScore *score)
{
  ...
  scores_list = _games_scores_backend_get_scores (cat->backend);
  ...
  if (_games_scores_backend_set_scores (cat->backend, scores_list) == FALSE)
    place = 0;
...
_games_scores_update_score_name (GamesScores * self, gchar * new_name, gchar * old_name)
{
  ...
  _games_scores_backend_set_scores (cat->backend, scores_list);
...
_games_scores_get (GamesScores * self)
{
  ...
  scores = _games_scores_backend_get_scores (cat->backend);
  /* Tell the backend that we won't be altering the scores so it
   * can release the lock. */
  _games_scores_backend_discard_scores (cat->backend);
...
GamesScoresBackend *
_games_scores_backend_new (GamesScoreStyle style,
                          char *base_name,
                          char *name)
{
  GamesScoresBackend *backend;
...
/**
 * _games_scores_backend_get_scores:
 ...
 */
GList *
_games_scores_backend_get_scores (GamesScoresBackend * self)
{
  gchar *buffer;
  gchar *eol;
  gchar *scorestr;
...
gboolean
_games_scores_backend_set_scores (GamesScoresBackend * self, GList * list)
{
  GList *s;
  GamesScore *d;
  gchar *buffer;
...
void
_games_scores_backend_discard_scores (GamesScoresBackend * self)
{
  games_scores_backend_release_lock (self);
}
