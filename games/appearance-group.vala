...
private class ThemeThumbnail : Gtk.Widget
{
    ...
    public ThemeThumbnail (Gtk.Widget child, string name)
    {
        this.accessible_role = Gtk.AccessibleRole.TOGGLE_BUTTON;
        ((Gtk.Accessible) this).update_property (Gtk.AccessibleProperty.LABEL, name, -1);
...
public class AppearanceGroup : Adw.PreferencesGroup
{
    ...
    public void add_theme (string name, Gtk.Widget thumbnail)
    {
        ...
        this.thumbnail_box.append (new ThemeThumbnail (thumbnail, name));
     ...
     *
     * `preview` is the main preview widget, it is switched with change_theme.
     *
     */
